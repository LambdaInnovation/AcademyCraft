 ...
 * Note that the data editing to this class have effect in both sides.
 * That is, the edit in client side will be sync to server side.
 * For that reason, you should be very careful so that when editing client
 * side data is always IN SYNC.
 * @author WeAthFolD
 */
@Registrant
@RegDataPart("loctele")
public class LocTeleData extends DataPart {
	...
	public void rename(int id, String newName) {
		...
		locationList.set(id, new Location(newName, 
				original.dimension, original.x, original.y, original.z));
		sync();
	}
