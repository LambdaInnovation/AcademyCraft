...
class EMContext(p: EntityPlayer) extends Context(p, ElectronMissile) {
  ...
  private val consumption: Float = lerpf(20, 15, exp)
  private val overload_attacked: Float = lerpf(61, 32, exp)
  ...
  private def s_onTick() = {
    if (!ctx.consume(overload, consumption)) terminate()
      ...
      val timeLimit: Int = lerpf(200, 400, exp).toInt
      if (ticks <= timeLimit) {
        ...
        if (ticks != 0 && ticks % 8 == 0) {
          val range: Float = lerpf(7, 12, exp)
          ...
          if (!active.isEmpty && !list.isEmpty && ctx.consume(overload_attacked, consumption_attacked)) {
            ...
            iter.remove()
            // client action
            sendToClient(MSG_EFFECT_SPAWN, VecUtils.entityPos(ball), VecUtils.add(VecUtils.entityPos(result), VecUtils.vec(0, result.getEyeHeight, 0)))
            // server action
            result.hurtResistantTime = -1
            val damage: Float = lerpf(14, 27, exp)
  ...
  private def s_onEnd() = {
    val cooldown: Int = MathUtils.clampi(100, 300, ticks)
